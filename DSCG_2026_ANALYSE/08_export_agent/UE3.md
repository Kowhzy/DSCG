# DSCG 2026 — UE3 — base de révision (générée le 24/09/2026)

## Consignes pour l'agent
- L'indice de priorité de révision (IPR, 0-100) hiérarchise l'effort de révision ; ce n'est PAS une probabilité que le thème tombe.
- Ne jamais raisonner « tombé en 2025 donc pas en 2026 » : le jury 2025 écrit qu'un point faible peut être réinterrogé dès l'année suivante (rapport 2025, p. 21).
- Ne jamais raisonner « absent depuis 5 ans donc il va tomber » : les zones peu interrogées sont signalées comme « à ne pas négliger », sans prédiction.
- Aucun point du programme n'est mineur (rapports du jury 2020, 2021, 2022, 2024, 2025) : les catégories basses indiquent un effort minimal, pas une impasse.
- Les corrigés sont « indicatifs ». Les entrées marquées ALERTE signalent un corrigé probablement erroné ou dépassé : ne pas faire apprendre ce chiffre ou cette règle sans vérification.
- Les valeurs « Non vérifié » ou « n.c. » sont des trous assumés : ne pas les combler par une invention.
- Session 2026 = dernière session sur le programme de l'arrêté du 13 février 2019 ; la réforme (arrêté du 4 août 2025) s'applique à partir de 2027.

## Épreuve
Écrit de 4 heures (coefficient 1,5 en 2019 selon la page de garde du sujet ; coefficient 2026 : non vérifié) ; 2 à 4 dossiers ; 2025 : 3 dossiers sur 20 points (8/6/6) [DSCG UE3 2025, page de garde]. Date 2026 : non vérifiée.

## Constats clés
- Référentiel applicable : programme de 2019 (1re session en 2020 ; selon le jury, le contenu de l'UE3 « n'a pas été profondément modifié avec la réforme ») ; la réforme de 2025 ne vaut qu'à partir de 2027.
- Le diagnostic stratégique (PESTEL, Porter, SWOT, ressources/compétences, business model) est présent dans 9 sessions sur 10 depuis 2016 (en toile de fond seulement en 2025). Le jury 2024 relève cependant des erreurs de choix d'outil : PESTEL ou SWOT au lieu des 5 forces pour le micro-environnement.
- Rupture 2024-2025 : le contrôle de gestion « fait part égale avec le management » [Rapport du jury 2024, p. 24 ; confirmé en 2025, p. 28]. Les calculs (écarts, facteur rare, masse salariale, prix de cession interne) sont redevenus centraux.
- Faiblesse n°1 signalée par le jury 2025 : « la décomposition des écarts et [l]es différents effets de la masse salariale ». La masse salariale était déjà ratée en 2022 : « pas compliqué mais peu de candidat[s] ont réussi » [Rapport du jury 2022, p. 10]. Les écarts figurent dans chacune des trois dernières sessions (2023, 2024, 2025) ; la masse salariale est calculée en 2017, 2022 et 2025 et la gestion des RH est en principal dans 5 sessions sur 10.
- Pilotage et performance : les techniques sont fréquentes (coûts cachés dans 4 sessions : 2018, 2020, 2022, 2023 ; tableau de bord social en 2022 et 2023), mais le programme les range en 3.2 (dysfonctionnements) et 4.1 (TBS). La rubrique 3.3 (KPI, tableaux de bord stratégiques, performance globale) n'est en principal qu'en 2020.
- Correction par rapport à la 1re version de cette étude : selon le texte officiel, la rubrique 2.2 porte sur l'audit, le contrôle interne et les risques ; elle n'a jamais été interrogée en principal. Les calculs de coûts (facteur rare en 2024 ; coûts, marges et seuil en 2019-2020) sont des techniques du DCG mobilisées au service de 2.5 (planification, choix de programme) et de 2.3 (tarification). Ils restent à maîtriser : voir le tableau des techniques (section 5.6).
- Le marketing (notamment digital) est la partie la plus abandonnée selon le jury 2022 : « La partie marketing n'est pas maîtrisée et souvent abandonnée » [Rapport du jury 2022, p. 10]. La rubrique 2.3 est en principal en 2020, 2022 et 2024 (en toile de fond en 2021).
- Chute du taux de réussite : de 50,14 % en 2024 à 17,28 % en 2025, avec une moyenne de 7,08 [Rapport du jury 2025, p. 10 et 19]. Le niveau technique et la contextualisation sont devenus discriminants.

## Priorités (IPR décroissant)
| Rubrique | Catégorie | IPR | Robustesse (rang min-max, 1er tiers) | Dernière | Notions officielles |
|---|---|---|---|---|---|
| 2.1 Analyse et choix stratégiques | PRIORITÉ MAXIMALE | 74.8 | 1-2, 7/7 | 2024 | diagnostic et chaîne de valeur, business model, risques, critères de performance, segmentation/DAS, stratégies génériques, recentrage/diversification, croissance interne/externe, alliances |
| 2.5 La planification et le diagnostic stratégique | PRIORITÉ MAXIMALE | 73.2 | 1-2, 7/7 | 2025 | CG et diagnostic, planification, plans prévisionnels (activité, financement, business plan), analyse d'un business model, intérêts et limites de la gestion budgétaire |
| 4.1 La gestion des ressources humaines | PRIORITÉ MAXIMALE | 64.4 | 3-4, 7/7 | 2025 | tableaux de bord sociaux, bilan social, écarts et effets de la masse salariale, GPEC, gestion de crise (négociation, médiation…) |
| 4.2 La gestion des compétences | PRIORITÉ TRÈS ÉLEVÉE | 55.3 | 4-5, 7/7 | 2025 | identification des compétences, formation, carrières, politique de rémunération, gestion des connaissances, apprentissage organisationnel |
| 2.4 La gestion du périmètre de l'entité | PRIORITÉ ÉLEVÉE | 43.4 | 3-7, 5/7 | 2025 | extension (acquisitions, fusions), réduction, coopération, réseau, prix de cession interne et de transfert |
| 2.3 Approche du marketing stratégique | PRIORITÉ ÉLEVÉE | 43.0 | 5-7, 1/7 | 2024 | leviers du marketing, communication multicanale, e-réputation, digitalisation et fidélisation, B to B, B to C, tarification dynamique (yield), big data |
| 1.2 Le positionnement du contrôle de gestion et l'identification du métier comme aide à la stratégie | PRIORITÉ ÉLEVÉE | 40.3 | 5-8, 1/7 | 2025 | rôle et place du CG selon les structures, travail d'information du contrôleur, CG et DD/RSE, éthique et déontologie |
| 2.6 Analyse des parties prenantes et structures de gouvernance | PRIORITÉ ÉLEVÉE | 37.3 | 6-9, 0/7 | 2025 | cartographie des parties prenantes, attentes et jeux de pouvoir, types de gouvernance, communication vers les parties prenantes |
| 3.3 Le contrôle de gestion et le pilotage stratégique | PRIORITÉ ÉLEVÉE | 32.0 | 6-10, 0/7 | 2020 | performance globale, FCS, KPI, tableaux de bord stratégiques |
| 3.2 Management et pilotage par les processus | À MAÎTRISER | 28.1 | 9-10, 0/7 | 2023 | cartographie des processus, progrès continu, coûts cibles, analyse de la valeur, dysfonctionnements organisationnels, usage des rapports d'audit |
| 3.1 Le management du changement | COMPLÉMENT | 19.2 | 11-12, 0/7 | 2018 | types et diagnostic du changement, styles et leviers, management en mode projet, suivi des projets, ordonnancement |
| 1.1 Évolution des modèles d'organisation | COMPLÉMENT | 15.8 | 11-14, 0/7 | 2019 | formes structurelles, structures par projet, réseaux, transversalité et processus, entreprise virtuelle, hybridation public/privé |
| 2.2 Place et rôle du contrôle de gestion stratégique | COMPLÉMENT | 14.8 | 12-14, 0/7 | jamais | CG face à l'audit et au contrôle interne, risques de l'entité, formes et champs de l'audit, communication financière, dimension culturelle des outils |
| 2.7 Contrôle de gestion et changement organisationnel | COMPLÉMENT | 12.6 | 12-15, 0/7 | jamais | CG interface stratégie/organisation, aide à l'évolution de la structure |
| 1.3 Le contrôle de gestion et les systèmes d'information et de communication | COMPLÉMENT | 8.3 | 13-15, 0/7 | jamais | PGI, transmission électronique de l'information, mégadonnées |

## Techniques les plus mobilisées
| Technique | Sessions 2016-25 | Depuis 2020 | Dernière |
|---|---|---|---|
| Note de synthèse | 4 | 4 | 2025 |
| Analyse d'écarts | 6 | 3 | 2025 |
| Parties prenantes | 5 | 3 | 2025 |
| PESTEL | 4 | 3 | 2024 |
| Coûts cachés (Savall-Zardet) | 4 | 3 | 2023 |
| 5 (+1) forces de Porter | 3 | 3 | 2024 |
| Stratégies génériques | 5 | 2 | 2024 |
| RSE / DD | 4 | 2 | 2025 |
| Gestion des compétences | 3 | 2 | 2025 |
| Masse salariale (effets niveau/masse/report) | 3 | 2 | 2025 |
| Marketing digital | 2 | 2 | 2024 |
| Prévisions de CA / résultat | 2 | 2 | 2024 |
| Tarification / grille de prix | 2 | 2 | 2024 |
| Tableau de bord social | 2 | 2 | 2023 |
| Diagnostic ressources / compétences | 3 | 1 | 2025 |
| Franchise / plateforme | 2 | 1 | 2024 |
| SWOT | 2 | 1 | 2021 |
| Outils de contrôle de gestion | 2 | 1 | 2021 |
| Coûts, marges, seuil de rentabilité | 2 | 1 | 2020 |
| Conduite de projet | 2 | 1 | 2020 |
| Prix de cession interne / de transfert | 1 | 1 | 2025 |
| Intégration verticale | 1 | 1 | 2025 |
| Coûts de transaction | 1 | 1 | 2025 |
| Optimisation sous contrainte (facteur rare) | 1 | 1 | 2024 |
| Scénarios | 1 | 1 | 2023 |
| Faire ou faire-faire (relocalisation) | 1 | 1 | 2023 |
| Plan de formation | 1 | 1 | 2023 |
| QVT, attractivité | 1 | 1 | 2023 |
| Marketing stratégique / mix | 1 | 1 | 2022 |
| GEPP / GPEC | 1 | 1 | 2022 |
| Knowledge management (SECI) | 1 | 1 | 2022 |
| Gouvernance (codes, agence, administrateurs) | 1 | 1 | 2022 |
| Océan bleu | 1 | 1 | 2021 |
| Business model (canvas) | 1 | 1 | 2021 |
| Intégration, fidélisation | 1 | 1 | 2021 |
| Coût du turnover | 1 | 1 | 2021 |
| Seuil de rentabilité d'une décision | 1 | 1 | 2021 |
| Rémunération (dont BSPCE) | 1 | 1 | 2021 |
| Diagnostic stratégique (démarche) | 1 | 1 | 2020 |
| Valeur client / offre | 1 | 1 | 2020 |
| Diagnostic des processus | 1 | 1 | 2020 |
| Tableau de bord de pilotage | 1 | 1 | 2020 |
| Performance globale | 1 | 1 | 2020 |
| Configurations de Mintzberg | 3 | 0 | 2019 |
| Modes de développement | 2 | 0 | 2019 |
| Entreprise libérée | 1 | 0 | 2019 |
| Chaîne de valeur | 1 | 0 | 2019 |
| Risques | 1 | 0 | 2019 |
| Conduite du changement | 1 | 0 | 2018 |
| Progrès continu | 1 | 0 | 2018 |
| Apprentissage organisationnel | 1 | 0 | 2018 |
| Rentabilité d'un plan d'actions | 1 | 0 | 2018 |
| Modèle de Greiner | 1 | 0 | 2017 |
| Valeur acquise (projet) | 1 | 0 | 2016 |

## Zones à ne pas négliger
- **1.3** — Jamais interrogé : ERP, mégadonnées, circulation de l'information au service du contrôle de gestion.
- **2.7** — Jamais interrogé en principal, alors que les jurys 2020-2021 citent « la mise en œuvre stratégique et la gestion du changement et des transformations ».
- **3.1** — Absent depuis 2018 (conduite du changement, styles, résistances, progrès continu vs projet). Les annales 2016 D2 et 2018 D2 sont les meilleures ressources.
- **2.2** — Jamais en principal : positionner le contrôle de gestion face à l'audit (interne/externe, légal/contractuel) et au contrôle interne, identifier les risques de l'entité, communication financière. Seulement en toile de fond (2019 D2 : risques ; 2020 D2 : audit vs diagnostic ; 2025 D3 : risque fiscal). Fiche de définitions et de liens CG/audit/CI.
- **3.2** — Coûts cachés et dysfonctionnements : 2018 D1, 2020 D2, 2023 D3 (et 2016 D2, 2022 D2 en toile de fond). Coûts cibles et analyse de la valeur, pourtant cités par le programme, n'ont jamais été interrogés sur la période.
- **1.2** — En principal en 2019 D1 (RSE), 2021 D4 (rôle du contrôleur en start-up) et 2025 D1 (RSE et redevance de pollution) : la RSE vue par le contrôle de gestion est revenue en 2025.

## Attendus des corrigés officiels (2020-2025)
- **2025 D1 — Q1 définir la RSE et ses axes** (n.c.) : Définition de la Commission européenne (intégration volontaire des préoccupations sociales et environnementales). Trois axes : social, environnemental, sociétal. — *Pièges :* Une définition sourcée est attendue, pas une paraphrase vague.
- **2025 D1 — Q2 calcul et analyse des écarts sur la redevance pollution** (n.c.) : Redevance 2024 = 2,10 × 53 270 × 3,2000208 = 357 976,73 € · 2023 = 1,97 × 55 509 × 3,1073267 = 339 794,66 € · écart global 18 182,07 € défavorable. Décomposition : écart sur redevance unitaire (Ru24 − Ru23) × V24 × Cp24 = 22 160,46 € D · écart sur pollution totale (V24·Cp24 − V23·Cp23) × Ru23 = −3 978,40 € F, lui-même = écart volume (V24 − V23) × Cp24 × Ru23 = −14 114,75 + écart concentration (Cp24 − Cp23) × V23 × Ru23 = 10 136,35. Autre schéma de décomposition accepté. — *Pièges :* Transposer la logique des écarts budgétaires (prix × quantité × composition) à un coût NON financier : c'est le cœur de la surprise 2025. Vérifier que la somme des sous-écarts redonne l'écart global. Qualifier chaque écart (F/D) et l'interpréter.
- **2025 D1 — Q3 analyse mensuelle (mois atypiques)** (n.c.) : Choisir et justifier trois mois (mai, novembre, décembre dans le corrigé) et relier les écarts à des causes opérationnelles. — *Pièges :* Justifier le choix des mois à partir des données, pas au hasard.
- **2025 D1 — Q4 parties prenantes et note de synthèse** (n.c.) : Freeman (1984). Salariés, actionnaires, fournisseurs, riverains, agriculteurs, collectivités · enjeux contradictoires à expliciter · note de synthèse structurée. — *Pièges :* Montrer les CONFLITS d'intérêts, pas seulement une liste. Respecter la forme « note ».
- **2025 D2 — Q1 diagnostic interne ressources et compétences** (n.c.) : Approche ressources/compétences : compétences managériales, fonctionnelles, transversales. Compétences distinctives (Hamel et Prahalad) : valeur, rareté, inimitabilité, non-substituabilité. — *Pièges :* Citer le modèle ET l'appliquer au cas.
- **2025 D2 — Q2 masse salariale : effets niveau, masse, report** (n.c.) : Effet niveau = 1,005 × 1,015 − 1 = 2,01 % · effet masse = 1 211,04 / 1 200 − 1 = 0,92 % · effet report = 1,0201 / 1,0092 − 1 = 1,08 % · contrôle : (1 + niveau) = (1 + masse) × (1 + report). — *Pièges :* Relation MULTIPLICATIVE, pas additive. Base 100 × 12 = 1 200. Les dates d'augmentation déterminent l'indice.
- **2025 D2 — Q3 comparer deux calendriers d augmentation** (n.c.) : Proposition de la direction +117 056 € contre +132 680 € pour celle des représentants du personnel : économie de 15 624 € · lien avec compétences et motivation. — *Pièges :* Chiffrer puis argumenter socialement (le moins coûteux n'est pas forcément retenu).
- **2025 D3 — Q1 intégration verticale amont** (n.c.) : Intégration verticale amont par croissance externe · coûts de transaction (Coase, Williamson). — *Pièges :* Nommer la théorie et l'appliquer.
- **2025 D3 — Q2 résultats des entités avec le PCI actuel** (n.c.) : Montesquieu : 9 900 € avant IS (7 425 € net) · Athéna : 106 812 000 € (90 790 200 € net). PCI de 1 599 € ≈ coût variable : transfert du bénéfice vers l'Irlande (IS 15 %) · taux de MCV 0,13 % contre 12,60 %. — *Pièges :* Calculer chaque entité séparément puis le groupe. Identifier l'effet fiscal du prix de transfert.
- **2025 D3 — Q3 nouveau PCI coût complet + marge** (n.c.) : PCI = coût complet 1 644,83 + 10 % = 1 809,31 € · résultat avant impôt du GROUPE inchangé, impôt total plus élevé. — *Pièges :* Un PCI déplace le résultat entre entités sans changer le résultat consolidé avant impôt : seul l'impôt change.
- **2025 D3 — Q4 arguments pour réviser le PCI** (n.c.) : Quatre registres : financier, fiscal (risque de redressement, principe de pleine concurrence), social (participation des salariés), sociétal (citoyenneté fiscale). — *Pièges :* Structurer par registre.
- **2024 D1 — 1.1 micro-environnement avec l outil pertinent** (n.c.) : L'outil attendu est les 5 (+1) forces de Porter (et non PESTEL/SWOT). Rivalité forte (indépendants, chaînes, franchises), clients à faible pouvoir mais versatiles, bailleur puissant, professeurs indépendants à faible pouvoir (marché saturé), entrants sans barrière (aucun diplôme requis), substituts nombreux (cours en ligne, applis, yoga en entreprise, puppy yoga…), État (exonération de cotisations, réglementation possible du métier). — *Pièges :* Le jury 2024 relève que de nombreux candidats ont utilisé PESTEL ou SWOT pour le micro-environnement : erreur d'outil.
- **2024 D1 — 1.2-1.3 grille tarifaire, CA et résultat prévisionnels** (n.c.) : Prix HT = TTC / 1,2 · CA HT prévisionnel = 66 837,50 € pour 208 inscrits (ex. Sat Nam 36 × 375 € + 60 × 362,50 € Early Bird) · résultat prévisionnel = 66 837,50 − 28 700 = 38 137,50 €. — *Pièges :* Raisonner en HT (TVA 20 %). Tous les chiffres recalculés : cohérents.
- **2024 D1 — 1.4 note de résultats et préconisations (20 lignes)** (n.c.) : Résultat 2022 = 54 226,67 − 31 738 = 22 488,67 € · objectif 45 000 € · économies possibles de 3 038 € seulement (charges fixes peu compressibles) → agir sur les prix · coût d'opportunité de l'Early Bird = 80 × 15 / 1,2 = 1 000 € · hausse du résultat de 15 648,83 € mais objectif non atteint. — *Pièges :* Respecter le format (note, 20 lignes) · conclure clairement sur l'objectif.
- **2024 D1 — 1.5 options stratégiques** (n.c.) : Matrice d'Ansoff : passer de la pénétration de marché au développement de produits · externalisation (sous-traitance de capacité des professeurs, fonctions support) et franchise envisageable · stratégies génériques : focalisation, à préciser en focalisation par différenciation (yoga traditionnel) plutôt que par les coûts. — *Pièges :* Distinguer stratégies globales (Ansoff, externalisation/intégration) et stratégies de domaine (Porter).
- **2024 D2 — 2.1 écarts sur CA expliqués par le micro et le macro-environnement** (n.c.) : CA 2023 = 64 680 € (+10 453,33 €, +19 %) mais écart défavorable de 2 045 € sur le budget (3 %) · écart sur prix −1 709,93 € (plus d'Early Bird, box moins chères) · écart sur quantité Sat Nam +2 570,31 €, Shanti −3 105 € · écart sur composition des ventes −1 005,68 € (cannibalisation vers les formules bon marché) · explication PESTEL : socioculturel (engouement), économique (inflation, pouvoir d'achat), technologique (cours en ligne). — *Pièges :* Décomposer écart sur prix / quantité puis volume global / composition · relier chaque écart à une cause d'environnement · remettre en cause la reconduction de l'Early Bird.
- **2024 D2 — 2.2 programme optimal de stages (facteur rare) et résultat marginal** (n.c.) : Ressource rare = 10 week-ends · MCV par stage : Little Nirvana 390 €, Nirvana 876 €, Nirvana Zen 567,50 € (après location de salle de 100 €) · MCV par week-end : 780 (2 Little Nirvana), 876, 567,50 → ordre Nirvana, Little Nirvana, Nirvana Zen · au moins une date par catégorie (2,5 week-ends) · programme 6 Little Nirvana, 6 Nirvana, 1 Nirvana Zen · résultat marginal = 8 163,50 € (pas de charge fixe supplémentaire). — *Pièges :* Classer par marge par unité de ressource rare, pas par marge unitaire · intégrer les minima imposés avant d'optimiser · le plafond de 6 Nirvana relève d'une contrainte du sujet (non vérifiée ici, le sujet n'étant pas disponible).
- **2024 D2 — 2.3 communication et marketing digital** (n.c.) : Communication actuelle outbound (flyers, affichettes) → approche inbound · leviers POEM : Paid (référencement payant, QR codes), Owned (site, un réseau social ciblé comme Instagram, ton adapté), Earned (likes, partages). — *Pièges :* Proposer un plan d'action concret et adapté à une TPE.
- **2024 D2 — 2.4 adhésion à une marketplace (approche marginale)** (n.c.) : CA supplémentaire = 64 680 × 7 % = 4 527,60 € · coûts = abonnement 39 × 12 = 468 € + commission 10 % du TTC = 12 % du HT (543,31 €) = 1 011,31 € · résultat marginal = 3 516,29 € → adhésion profitable. — *Pièges :* Piège : une commission de 10 % sur le TTC représente 12 % du HT.
- **2024 D2 — 2.5 adhésion à un réseau de franchise** (n.c.) : Définition de la franchise · avantages (assistance, indépendance) et inconvénients (normes, droit d'entrée, redevances) · Yoga World : investissement 500 000 €, droit d'entrée 35 000 €, redevance 7 % (17 500 € sur 250 000 €), philosophie incompatible · Studio Zen : investissement 5 000 €, redevance 1 500 €/an (3,75 % du CA prévu, 2,32 % du CA actuel) mais CA prévu 40 000 € < 64 680 € actuel → conclusion : n'adhérer à aucun réseau. — *Pièges :* Relier les chiffres à la vision de la dirigeante (yoga traditionnel, vie personnelle).
- **2023 D1 — Q1 PESTEL opportunités/menaces, facteurs pivots, scénarios** (n.c. (dossier 30 pts)) : PESTEL appliqué à l'éolien · identifier les variables pivots (acceptabilité, dérèglement climatique, soutien public) · construire deux scénarios (favorable, défavorable) · conclure sur l'incertitude (investissement = pari, activité d'exploration). — *Pièges :* Le corrigé sanctionne la liste « à la Prévert » : il faut hiérarchiser et conclure.
- **2023 D1 — Q2 5 (+1) forces de Porter** (n.c.) : 5 forces ou 5+1 acceptées · schéma conforme à Porter (axe horizontal fournisseurs-concurrents-clients) · clients très puissants (oligopole, 9/10), État 8/10, fournisseurs 6/10, concurrence et entrants 5/10, substituts 1/10 (notes facultatives). — *Pièges :* Analyser le marché de l'entreprise (composants) et non celui de ses clients. Ne pas oublier une force essentielle.
- **2023 D1 — Q3 synthèse environnement général et concurrentiel** (n.c.) : Demi-page combinant PESTEL et Porter : forte incertitude, forces principales = clients et pouvoir politique. — *Pièges :* Respecter la longueur imposée.
- **2023 D2 — Q1 écarts sur CA et sur MCV par gamme (volume, prix/coût)** (n.c. (dossier 35 pts)) : Écart CA total ≈ +432 k€ mais MCV −376 k€ · Ciel'Ouvert : écart volume CA +1 124, prix −663 · MCV volume +303, coût −945 · Brise : volume −3 056 (CA) et −1 151 (MCV) · conclusion : deux gammes problématiques (Ciel'Ouvert par les coûts, Brise par les volumes). — *Pièges :* Écart volume = ΔQ × marge (ou prix) de référence · écart prix/coût = ΔP × Q réelle. Conclure en note au COPIL.
- **2023 D2 — Q2 cartographie des parties prenantes** (n.c.) : Freeman · pouvoirs publics FR/TR, direction, sous-traitant turc, commune, salariés et syndicats, nouveaux fournisseurs · présentation matricielle conseillée. — *Pièges :* Intégrer les dimensions extra-comptables et coûts cachés.
- **2023 D2 — Q3 gain économique de la relocalisation** (n.c.) : MCV objectif 6 196 k€ contre 4 620 k€ réalisés : gain 1 576 k€ · frais de mise au point (25 % matière + personnel sur 3 mois) 927 k€ · gain net 1re année ≈ 650 k€ · décision pertinente. — *Pièges :* Le gain du corrigé compare un objectif à 1 000 000 pièces au réalisé à 947 177 : il inclut donc un effet volume. Le signaler dans la copie.
- **2023 D2 — Q4 raisons de l internalisation** (n.c.) : Critère économique (question 3) + critères extra-économiques : image Made in France, recrutement, soutien public, maîtrise logistique et qualité, souveraineté. — *Pièges :* Faire/faire-faire : croiser chiffré et qualitatif.
- **2023 D3 — Q1 définir la compétence et méthode d évaluation** (n.c. (dossier 35 pts)) : Savoir + savoir-faire + savoir-être · évaluation par le manager de proximité · grille de compétences Savall-Zardet (carré noir/semi-noir/blanc/tiret) · repérer forces et vulnérabilités. — *Pièges :* Méthode adaptée à une PME industrielle.
- **2023 D3 — Q2 formation et plan de formation** (n.c.) : Grille prospective des compétences de demain · plan de formation négocié managers/salariés · mobilité interne, communautés de pratiques, recrutement externe. — *Pièges :* Distinguer compétences actuelles et futures.
- **2023 D3 — Q3 six domaines de QVT et attractivité** (n.c.) : Conditions de travail · organisation du travail · 3C (communication-coordination-concertation) · gestion du temps · formation · mise en œuvre stratégique (dont rémunération). — *Pièges :* Six domaines du diagnostic socio-économique (ISEOR).
- **2023 D3 — Q4 coûts cachés et 3 indicateurs** (n.c.) : Coûts non pris en compte par le SI comptable · composantes : surtemps, sursalaires, non-production, surconsommations, risques · 5 dysfonctionnements : absentéisme, AT/MP, rotation, défauts de qualité, écarts de productivité directe. — *Pièges :* Les 3 indicateurs sont à choisir parmi ces 5 domaines.
- **2023 D3 — Q5 construire un TBS** (n.c.) : Indicateurs de résultat qualitatifs, quantitatifs, financiers (coûts cachés) sur les 5 domaines + indicateurs d'action (compétences, formation, recrutement) · responsables et périodicité. — *Pièges :* Présentation tableau : objectif, indicateur, cible, réalisé, écart, responsable.
- **2022 D1 — Q1-Q2 PESTEL et 5+1 forces (Gant Maille)** (n.c. (dossier 40 pts selon sujet)) : PESTEL (plan de relance, Covid, délocalisations, pénurie de couturières) · 5+1 forces · synthèse opportunités/menaces. — *Pièges :* Toujours conclure sur O/M.
- **2022 D1 — Q3 stratégie générique** (n.c.) : Différenciation vers le haut (Made in France, qualité) · matrice de Porter 2×2.
- **2022 D1 — Q4-Q6 B to B / B to C, marketing stratégique, leviers** (n.c.) : Enjeux B to C · étapes du marketing stratégique (analyse, objectifs SMART, segmentation-ciblage-positionnement) · 4P puis 7P (people, process, physical evidence) · e-réputation. — *Pièges :* Leviers « modernes » = digital et fidélisation.
- **2022 D2 — Q1 enjeux de la GEPP** (n.c. (dossier 35 pts selon sujet)) : Définition · enjeux stratégiques, économiques, sociaux, légaux (négociation triennale ≥ 300 salariés). — *Pièges :* Gant Maille n'est pas soumise à l'obligation : le dire.
- **2022 D2 — Q2-Q3 diagnostic RH et knowledge management** (n.c.) : Pyramide des âges (champignon), sous-effectif de couturières, manque de modéliste et de marketing · formation, tutorat, recrutement · modèle SECI de Nonaka et Takeuchi (socialisation, extériorisation, combinaison, intériorisation). — *Pièges :* Appliquer chaque étape SECI au cas.
- **2022 D2 — Q4 construire un TBS** (n.c.) : Tableau problème RH / moyen d'action / indicateur / cible / réalisé / écart · turnover, recrutements, départs en retraite < 5 ans, formation.
- **2022 D2 — Q6 masse salariale 2022 (méthode indiciaire)** (n.c.) ⚠ ALERTE : Indices : 1 221 (hausse 3 % au 1/6) et 1 242 (hausse 6 %) pour base 1 200. Masse 2021 = 516 900 €. Corrigé : 529 707,75 € et coût visible 12 807 €. RECALCUL : 1 242 / 1 200 = 1,035 (et non 1,034) et 46 800 × 1,0175 = 47 619 (et non 47 419) → masse 2022 = 529 935,75 € · coût visible ≈ 13 035,75 €. — *Pièges :* ALERTE : erreur de coefficient dans le corrigé (1,034 au lieu de 1,035) et total incohérent avec ses propres lignes. Retenir le raisonnement, pas le chiffre.
- **2022 D2 — Q7 coûts cachés et décision d augmenter les salaires** (n.c.) : Dysfonctionnements (démotivation, absentéisme, rotation, qualité, recrutement) → surtemps, sursalaires, non-production, surconsommations · décision justifiée si coûts cachés > coût visible de l'augmentation (sécurité : ≥ 2 × ≈ 25 000 €). — *Pièges :* Raisonnement « balance économique » réutilisable.
- **2022 D3 — Q1 parties prenantes, objectifs contradictoires** (n.c. (dossier 25 pts selon sujet)) : Freeman, Carroll · primaires/secondaires, internes/externes · tableau PP/attentes · conflits (client qualité vs prix, etc.) · gouvernance partenariale.
- **2022 D3 — Q2 gouvernance familiale** (n.c.) : Avantages/inconvénients (longévité, stabilité vs conflits, succession) · évolutions : holding familiale, dirigeant externe, pacte d'associés, actionnariat salarié, investisseurs.
- **2022 D3 — Q3 RSE, DD et processus RSE** (n.c.) : Livre vert UE 2001 · rapport Brundtland 1987 · dialogue interne/externe · ISO 26000 · objectifs SMART.
- **2021 D1 — Q1 SWOT et FCS** (24 pts (dossier)) : Intérêt de la SWOT · tableau forces/faiblesses/opportunités/menaces · FCS : tiers de confiance, grades de qualité, sélection des reconditionneurs.
- **2021 D1 — Q2-Q3 stratégie océan bleu et durabilité** (n.c.) : Innovation de valeur centrée sur la relation client · exemples (Tediber, Emma, Nespresso) · risque de basculer en océan rouge (Fnac, Cdiscount, Apple). — *Pièges :* Fournir au moins deux exemples.
- **2021 D2 — Q1-Q3 business model canvas** (24 pts (dossier)) : Vocation du BM · canvas en 9 blocs complété (annexe à rendre) · pierre angulaire : tiers de confiance et algorithme de sélection · risques : défaillance des reconditionneurs. — *Pièges :* L'annexe complétée doit être rendue avec la copie.
- **2021 D3 — Q1-Q3 intégration, communication interne, happiness manager** (26 pts (dossier)) : Intégration présentielle et à distance · générations X/Y, motivations intrinsèques · rôle du happiness manager.
- **2021 D3 — Q4 seuil de rentabilité du happiness manager** (n.c.) : Coût 85 000 € (50 000 + 40 % + 15 000) · coût de renouvellement 7 600 €/départ (recrutement 10 % + intégration 10 % de 38 000) · seuil 85 000 / 7 600 = 11,18 → 12 départs évités · turnover de 15 % à 11 %. — *Pièges :* Arrondir au nombre entier supérieur.
- **2021 D4 — Q1 BSPCE et fidélisation** (26 pts (dossier)) : Bons attribués gratuitement, exercice à prix privilégié, dilution, gain en capital, liquidité à l'introduction en bourse.
- **2021 D4 — Q2 évolution du CA et rentabilité 2016-2021** (n.c.) ⚠ ALERTE : +220 % = coefficient 3,2 (736 000 = 230 000 × 3,2) · CA = taux de commission × volume d'affaires · masse salariale = effectif × salaire × 1,40 × 12. ALERTE : dans le tableau du corrigé, les marges 2020 (−26 296) et 2021 (+28 460) ne correspondent pas à la somme des lignes affichées (−28 400 et −47 620). — *Pièges :* ALERTE : Piège « +220 % ≠ ×2,2 ». Refaire l'addition : incohérence interne du corrigé, non vérifiée contre le sujet.
- **2021 D4 — Q3-Q4 résultats de start-up et outils de CG** (n.c.) : Pertes normales en phase de croissance · choisir ≥ 3 outils parmi 7 avec nature, périodicité, justification (trésorerie, TB commercial, budget, BSC…).
- **2020 D1 — Q1-Q3 diagnostic stratégique (SER-START)** (60 pts (dossier)) : Démarche : outil (5+1 forces, SWOT, PESTEL), forces/faiblesses, positionnement · note de synthèse.
- **2020 D1 — Q4 coûts, marges, seuils de rentabilité par activité** (n.c.) : Coûts unitaires selon deux clés (ex. location 8,16 €/h gestionnaire ou 31,48 €/place) · MCV globale 89 000, CF 82 000, résultat 7 000 · SR global = 82 × 117 / 89 ≈ 107 k€ · marge sur coûts spécifiques : service I&D −7 600 €. — *Pièges :* Discuter la pertinence des clés de répartition et des inducteurs.
- **2020 D1 — Q5-Q11 tarification, valeur, packs, préconisations** (n.c.) : Valeur pour le client : pack 1 gain 200 €, pack 2 perte 3 000 € (non viable) · tarification dynamique · nouveau pack (ex. 3 mois + 3 prestations à 3 000 € contre 3 600 €) · ≥ 3 préconisations justifiées.
- **2020 D2 — Q1-Q3 diagnostic des processus et coûts des dysfonctionnements** (40 pts (dossier)) : Audit interne vs diagnostic interne · 6 thèmes socio-économiques · effet-miroir · ≥ 5 dysfonctionnements · coûts cachés (surtemps, sursalaires, surconsommations, non-production, non-création de potentiel, risques).
- **2020 D2 — Q4-Q5 groupe de projet et balance économique** (n.c.) : Groupe restreint + plénier · plans d'actions · balance économique : coût des solutions < coût des dysfonctionnements · surcoût annuel chiffré 60 200 €.
- **2020 D2 — Q6-Q8 tableau de bord de pilotage et performance globale** (n.c.) : Définitions TB et indicateur · domaines économique, social, environnemental · un indicateur par domaine justifié.

## Observations du jury
- [2020, p. 7] Sujet long : « Le critère de bonne gestion du temps des analyses est devenu récurrent sur le sujet UE3 ». → S'entraîner en temps réel (4 h) avec beaucoup d'annexes.
- [2020, p. 7-8] Points majeurs : extra-financier, investissements immatériels et humains, mise en œuvre stratégique, gestion du changement, « calculs de coûts/performances fondamentaux », incertitude, choix entre options, scénarios. → Couvrir changement + RH + calculs de coûts ; raisonner en scénarios.
- [2020, p. 8] Épreuve « plus axée sur une réflexion ancrée sur des cas réels […] que sur des calculs bruts et des définitions non contextualisées ». → Toujours appliquer l'outil au cas.
- [2022, p. 9] Sujet accessible (stratégie, GRH, enjeux contemporains) ; un volet marketing, notamment digital, « qu'il convient d'intégrer dans son programme de révision ». → Intégrer le marketing digital aux révisions.
- [2022, p. 9] « l'épreuve constitue une étude de cas et pas des questions de cours » : contextualiser, ne pas paraphraser les annexes, être analytique et non descriptif. → S'entraîner à la démarche d'étude de cas.
- [2022, p. 10] « La partie marketing n'est pas maîtrisée et souvent abandonnée » ; lacunes sur la RSE et la politique RH ; « Les notions de stratégie ne sont pas maîtrisées ». → Marketing (y compris digital), RSE, politique RH et stratégie : fiches + application au cas.
- [2022, p. 10] « Le calcul de la masse salariale n'était pas compliqué mais peu de candidat[s] ont réussi par manque de maîtrise ». → Masse salariale : faiblesse signalée en 2022 puis de nouveau en 2025.
- [2021, p. 8] Sujet start-up en 4 dossiers (diagnostic, business model, RH, rôle du contrôleur) ; « assez long » ; contenu du programme UE3 « pas […] profondément modifié avec la réforme ». → Les annales antérieures à 2020 restent exploitables en UE3.
- [2024, p. 24] « le contrôle de gestion fait part égale avec le management » ; compétences de haut niveau (analyser, évaluer, créer). → Le contrôle de gestion chiffré pèse désormais la moitié du barème.
- [2024, p. 25] Réponses descriptives, récitation, « fréquent manque de contextualisation » ; méconnaissance des TPE et des services. → Contextualiser ; s'entraîner sur des TPE et des services.
- [2024, p. 26] « de nombreux candidats ont utilisé le PESTEL ou SWOT au lieu des forces de Porter pour analyser le micro-environnement ». → Choisir l'outil selon le niveau d'analyse (macro vs micro).
- [2024, p. 26] « peu de candidats ont compris qu'il fallait utiliser la méthode du facteur rare ». → Refaire des exercices d'optimisation sous contrainte.
- [2025, p. 28] Rééquilibrage CG/management « confirmé » ; concevoir un schéma d'analyse d'écarts, analyser masse salariale et prix de transfert ; RSE traitée par le double prisme. → Savoir construire soi-même une décomposition d'écarts.
- [2025, p. 29] Consignes non respectées (concepts non définis alors que demandé) ; récitation « comme en 2024 ». → Définir chaque concept demandé avant de l'appliquer.
- [2025, p. 29-30] « De nombreux candidats ne maîtrisent pas les techniques de base, qu'il s'agisse de la décomposition des écarts et des différents effets de la masse salariale » ; calcul d'une redevance à partir d'une formule simple raté. → PRIORITÉ : écarts (y compris réinterprétés) et effets niveau/masse/report.
- [2025, p. 30] Conseils 2026 : maîtriser et réinterpréter les techniques ; exploiter calculs et concepts dans une argumentation ; articuler management et CG. → Chaque calcul doit déboucher sur une analyse et une préconisation.
- [2025, p. 18-19] Taux de réussite UE3 : 50,14 % (2024) → 17,28 % (2025) ; moyenne 9,65 → 7,08. → Session 2025 très sélective : le niveau technique est discriminant.

## Annales recommandées
- **2025 (sujet entier, 4 h)** — À refaire absolument : schéma d'écarts à construire (redevance), effets sur la masse salariale, compétences distinctives, prix de transfert, parties prenantes. Ce sont les faiblesses n°1 du jury 2025.
- **2024 (sujet entier)** — Porter pour le micro-environnement (et non PESTEL), grille tarifaire, écarts de CA, facteur rare, marketing digital, marketplace, franchise ; TPE de services.
- **2023 D2** — Écarts sur marge (volume, prix/coût), gain d'une relocalisation (analyse différentielle), cartographie des parties prenantes.
- **2023 D3 + 2022 D2** — Compétences, formation, GEPP, knowledge management, coûts cachés, tableau de bord social, masse salariale.
- **2017 D3 (ancien programme)** — Masse salariale : variation par catégorie, écarts de salaire/composition/effectif, effets de niveau, de masse et de report. La référence technique sur les effets.
- **2019 D3 (ancien programme)** — Décomposition des écarts de marge de production, contributions par canal, franchise.
- **2020 D2 + D1 partie B** — Diagnostic des processus, coût des dysfonctionnements, tableau de bord et indicateurs de performance globale ; coûts, seuil, valeur client.
- **2018 D2 + 2016 D2** — Conduite du changement, styles, progrès continu ; pilotage de projet par la valeur acquise et les écarts. Zones peu interrogées récemment.
- **2021 D2 + D4** — Business model canvas, outils de contrôle de gestion en start-up (rôle du contrôleur).

## Dossiers d'annales 2016-2025
| Session | D | Intitulé | Pts | Rubriques P | Rubriques s | Techniques |
|---|---|---|---|---|---|---|
| 2025 | 1 | Incidence de la RSE et coût de la pollution (Tanneries Montesquieu) | 8/20 | 2.5|2.6|1.2 | 3.3 | RSE|ecarts|parties_prenantes|note_synthese |
| 2025 | 2 | Gestion des compétences et des ressources humaines | 6/20 | 4.1|4.2 | 2.1 | diagnostic_ressources_competences|competences|masse_salariale |
| 2025 | 3 | Gestion des prix de transfert | 6/20 | 2.4 | 2.1|2.2|2.6 | PCI|integration_verticale|couts_transaction |
| 2024 | 1 | Analyse de la situation initiale et première décision (école de yoga Yoga Life, TPE de services) | n.v./n.v. | 2.1|2.3|2.5 | 2.4 | Porter_5F|tarification|previsions_resultat|strategies_generiques|note_synthese |
| 2024 | 2 | Analyse des résultats et nouveaux projets (écarts, facteur rare, marketing digital, marketplace, franchise) | n.v./n.v. | 2.5|2.3|2.4 | 3.3|2.1 | ecarts|facteur_rare|digital|franchise_plateforme|PESTEL |
| 2023 | 1 | Analyse de la stratégie commerciale (gamme Brise) | 30/100 | 2.1 |  | PESTEL|Porter_5F|scenarios|note_synthese |
| 2023 | 2 | Analyse stratégique (relocalisation/internalisation Ciel'Ouvert) | 35/100 | 2.5|2.4|2.6 |  | ecarts|faire_faire_faire|parties_prenantes|note_synthese |
| 2023 | 3 | Organisation et management des ressources humaines | 35/100 | 4.2|4.1|3.2 |  | competences|formation|QVT_attractivite|couts_caches|TBS |
| 2022 | 1 | Stratégie et marketing (Gant Maille) | 40/100 | 2.1|2.3 |  | PESTEL|Porter_5F|strategies_generiques|marketing|digital |
| 2022 | 2 | Ressources humaines et compétences | 35/100 | 4.1|4.2 | 3.2|1.1 | GEPP|KM|TBS|masse_salariale|couts_caches |
| 2022 | 3 | Gouvernance et RSE | 25/100 | 2.6 | 1.2 | parties_prenantes|gouvernance|RSE |
| 2021 | 1 | Diagnostic stratégique (Back Market) | 24/100 | 2.1 |  | SWOT|ocean_bleu |
| 2021 | 2 | Présentation et analyse du business model | 24/100 | 2.1|2.5 | 2.3 | business_model |
| 2021 | 3 | Ressources humaines et modélisation des équipes | 26/100 | 4.1|4.2 | 1.1 | integration_fidelisation|cout_turnover|seuil_rentabilite |
| 2021 | 4 | Opportunité pour un contrôleur de gestion d'intégrer Back Market | 26/100 | 1.2|2.5|4.2 | 3.3 | remuneration|previsions_resultat|outils_CG |
| 2020 | 1 | Diagnostic stratégique et préconisations (SER-START, crise Covid) | 60/100 | 2.1|2.3|2.5 |  | diagnostic_strategique|couts_marges_seuil|tarification|valeur_client|note_synthese |
| 2020 | 2 | Amélioration des processus et de leur pilotage | 40/100 | 3.2|3.3 | 3.1|2.2 | diagnostic_processus|couts_caches|conduite_projet|tableau_de_bord|performance_globale |
| 2019 | 1 | Structure et engagement sociétal (Picture Organic Clothing) | 31/100 | 1.1|1.2 | 3.3|2.6 | Mintzberg|entreprise_liberee|RSE |
| 2019 | 2 | Analyse stratégique | 44/100 | 2.1|4.2 | 2.2 | strategies_generiques|chaine_valeur|diagnostic_ressources_competences|competences|risques |
| 2019 | 3 | Analyse des marges et prospective | 25/100 | 2.4|2.5 | 2.1 | modes_developpement|franchise_plateforme|ecarts|couts_marges_seuil |
| 2018 | 1 | Gouvernance, stratégie et diagnostic de la situation de changement (ESO) | 60/100 | 2.1|2.6|3.2 | 1.2|3.3|3.1 | strategies_generiques|parties_prenantes|RSE|couts_caches |
| 2018 | 2 | Conduite du changement et amélioration de la productivité | 40/100 | 3.1|3.2 | 4.2|2.7 | conduite_changement|progres_continu|apprentissage_organisationnel|rentabilite_actions |
| 2017 | 1 | Analyse stratégique de BlaBlaCar | 20/100 | 2.1 |  | PESTEL|SWOT |
| 2017 | 2 | Organisation et développement de BlaBlaCar | 30/100 | 1.1|2.1 | 2.4|2.7 | Mintzberg|Greiner|modes_developpement |
| 2017 | 3 | Politique RH et maîtrise de la masse salariale | 50/100 | 4.1 |  | masse_salariale|ecarts |
| 2016 | 1 | Stratégie, structure et responsabilité sociétale (CRFCK, association) | 58/100 | 2.1|1.1|2.6 | 3.3 | diagnostic_ressources_competences|strategies_generiques|Mintzberg|parties_prenantes |
| 2016 | 2 | Gestion de projet de changement | 42/100 | 3.1|2.5 | 1.2|3.2 | conduite_projet|valeur_acquise|ecarts|outils_CG |
